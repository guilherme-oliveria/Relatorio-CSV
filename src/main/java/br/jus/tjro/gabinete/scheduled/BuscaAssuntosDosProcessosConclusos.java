...
@Component
public class BuscaAssuntosDosProcessosConclusos {

    @Autowired
    private ProcessoAssuntoService service;
    
    private final Logger looger = LoggerFactory.getLogger(BuscaAssuntosDosProcessosConclusos.class);


    @Transactional(propagation = Propagation.REQUIRES_NEW)
    public Boolean executaPorProcesso(Processo processo) {
        try {
            service.importaOuAtualizaAssuntosDoProcesso(processo);
            return true;
        } catch (Exception e) {
